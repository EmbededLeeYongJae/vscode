...
// 명령(action), reducer, store(상태관리를 일원화하기)
// 상태관리 흐름 : action > reducer > store > UI
...
function Plus1(){
    ...
    return(
        <div>
            ...
            <p><button onClick={() => dispatcher({type: 'plus1'})}> + </button>
                : {selector}</p>
        </div>
    );
}
    function Plus2(){
        ...
        return(
            <div>
                ...
                <p><button onClick={() => dispatcher({type: 'plus2'})}> + </button>
                    : {selector}</p>
            </div>
        );
...
function Plus3(){
    ...
    return(
        <div>
            ...
            <p><button onClick={() => dispatcher({type: 'plus3'})}> + </button>
                : {selector}</p>
        </div>
    );
}