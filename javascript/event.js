// Event(이벤트)
// - 자바스크립트는 이벤트기반프로그래밍 모델 (Event Driven Programming Model)
// : 이벤트가 발생하는 이벤트객체가 이벤트 핸드럴(=이벤트리스너=이벤트골백=콜백)에 
//   인자로 전달이 되서 이벤트 처리를 하는 프로그래밍 원덜
// - 자바스크립트가 감지할 수 있는 모든 것은 이벤트
// : ex)윈도우가 열림/닫힘, 마우스 누름,뗌,클릭, 키보드 누름/뗌, 데이터입력
